...
def start():
    ...
    # getNGO()
    return render_template("login.html")    # if 'user' in session:
...
def login():
    if 'user' in session:
        ...
        if int(content['role'])==1:
            ...
            print("Inside volunteer")
            pw_hash = db.volunteers.find_one({"username":username},{"pass":1,"_id":0,"name":1})
            if pw_hash:
                if bcrypt.check_password_hash(pw_hash['pass'],password):
...
def toJson(data):
    return Response(json_util(data.dumps), mimetype='application/json')